...
and/or redefines many internal macros and, with the evolution of
\texttt{beamer} and the \LaTeX\ kernel, these modifications became brittle.
Over time a number of issues have cropped up. Most of the issues are minor and
...
\texttt{Metropolis} being unmaintained, however, these contributions have not
been incorporated.
...
features and contributions from the community. I have made some design changes,
but these have largely been made to make the design more accessible and to
bring the behavior of the theme closer to standard \texttt{beamer} behavior.
Finally, I have also made a number of internal code improvements to make the
theme more robust and maintainable going forward. Users of \texttt{Metropolis}
should find it easy to switch to \texttt{Moloch}, whilst at the same time
benefit from the bug fixes, new features, and improved stability.

Since neither \texttt{Metropolis} nor \texttt{Moloch} have been introduced in
the \TUB\ before, I will begin to do just that by introducing \texttt{Moloch}.
...
changes from the lighter and vivid
...
Metropolis introduced a bit of custom logic to handle block environments. In
particular, filled blocks environments were modified such that the main body
text (for the frame) aligns with the boundaries of the box and not the text
inside the box (which is the default behavior in beamer). See
Figure~\ref{fig:block-comparison} for a comparison.
...
\texttt{Metropolis} uses \cs{textbullet} as the symbol for all levels of the
itemize environment. The problem with \cs{textbullet} is that it does not work
with every font. Notably, for instance, it does not work with \titleref{Latin
  Modern}~(\tbcode{lmodern}), and instead is replaced by squares in that font.
...
like \latexinline{$\bullet$}, but then the problem is only shifted to depend on
the math font instead.
...
environment~(Figure~\ref{fig:itemize-comparison}).
