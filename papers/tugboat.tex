...
\begin{abstract}
  ...
  \texttt{Metropolis}, which is no longer actively maintained. Moloch aims to fix
  outstanding issues with \texttt{Metropolis} while keeping the original design
  mostly intact. This article introduces \texttt{Moloch} and summarizes the
  changes made compared to \texttt{Metropolis}.
\end{abstract}
...
presentations in addition to enabling useful features such as progress bars and
(what we think are) sane defaults for presentations.

The problem is that \texttt{Metropolis} is no longer actively
maintained\Dash{}its latest updates date six years back. Internally, it patches
or redefines many internal macros and, with the evolution of \texttt{beamer}
and the \LaTeX\ kernel, these modifications have become brittle. Over time a
number of issues have cropped up. Most of the issues are minor and corrigible
through workarounds, but our preambles have grown increasingly cluttered with
such patches. Moreover, a number of users have requested features, while others
have offered contributions to help improve the theme. \texttt{Metropolis} being
unmaintained, however, these contributions have not been incorporated.
...
features and contributions from the community. We have made some design
changes, but these have largely been made to make the design more accessible
and to bring the behavior of the theme closer to standard \texttt{beamer}
behavior. Finally, we have also made a number of internal code improvements to
make the theme more robust and maintainable going forward. Users of
...
the \TUB\ before, we will begin by introducing \texttt{Moloch}. Users of
\texttt{Metropolis} may wish to skip ahead to
Section~\ref{sec:metro-vs-moloch}, which summarizes the changes made in
\texttt{Moloch}.

\section{Getting Started with Moloch}

Moloch is a standard \texttt{beamer} theme and is enabled like any other theme.
...
\texttt{beamer} title slide\footnote{But if you prefer the default
  \texttt{beamer} one, then \texttt{Moloch} has the option to use that instead}.
...
Like many other \texttt{beamer} themes, Moloch is split into a color theme,
font theme, and inner and outer themes. Here, we assume that the full Moloch
...
\begin{figure}[htb]
  ...
  \caption{%
    ...
    \latexinline{\molochset{sectionpage=none}}.}\label{fig:section-page-default}
\end{figure}

\subsection{Standout Frames}

\texttt{Moloch} (as well as \texttt{Metropolis}) supports special
\emph{standout} frames, which are intended as a mean for highlighting important
frames in the presentation. They are created by specifying a \tbcode{standout}
key to the \tbcode{frame} environment. For example, the following code snippet
...
\begin{figure}[tpb]
  ...
  \caption{%
    A standout frame created via \latexinline{\begin{frame}[standout] ... \end{frame}}.}\label{fig:standout-frame}
\end{figure}
...
\section{How to Transition from Metropolis to Moloch}
\label{sec:metro-vs-moloch}

As mentioned in the introduction, \texttt{Moloch} is a fork of
\texttt{Metropolis} with the goal of fixing outstanding issues while keeping
the original design mostly intact. In this section, we summarize the main
changes made in \texttt{Moloch} compared to \texttt{Metropolis}, explain the
motivation behind them, and offer guidance for users of \texttt{Metropolis} who
wish to transition to \texttt{Moloch}.

Many of the changes in \texttt{Moloch} are internal code improvements and
mitigations of bugs and we will not focus on these here, but rather on
user-visible changes.

One important difference is that we have tried to make \texttt{Moloch} more
consistent with standard \texttt{beamer} behavior where possible. The leading
principle has been to not re-invent the wheel when \texttt{beamer} already
provides the desired functionality. This has led to some changes in behavior
compared to \texttt{Metropolis}. Another related change is to avoid redefining
internal \texttt{beamer} macros. This was a source of brittleness in
\texttt{Metropolis} that led to upstream workarounds in \texttt{beamer} to
avoid breaking changes. By relying on standard \texttt{beamer} mechanisms
instead, we hope to make \texttt{Moloch} more robust going forward.

\subsection{New Example Color}

While we generally find \texttt{Metropolis} well-designed, we felt that the
default color for example blocks (green) could be difficult to read for some
users, especially those with color vision deficiencies. Therefore, we have
changed from the lighter and vivid
\textcolor{metropolisGreen}{green}~(\textcolor{metropolisGreen}{\rule{1.2ex}{1.2ex}})
used for example blocks in \texttt{Metropolis} to a darker, but less saturated
\textcolor{molochGreen}{teal}~(\textcolor{molochGreen}{\rule{1.2ex}{1.2ex}}) in
\texttt{Moloch}. Users who prefer the original green color can easily revert to
it by adding the following lines.
...
Erik Spiekermann). And on Linux systems on \LuaLaTeX\ and \XeLaTeX, the package
automatically attempts to find, configure, and use this font. While we
appreciate the design of \texttt{Fira Sans}, we also feel that the font choice
...
instead leave the choice of font to the user. For users who still wish to use
\texttt{Fira Sans}, the following lines will help set this up.
...
main font. Here is an example configuration that closely mimics the original
...
\texttt{Metropolis}, you do not need to sprinkle \cs{medskip} (or whatever you
use for paragraph spacing) in your slides to have them neatly separated.
...
the different paragraph spacing.

For those reasons, we have removed this setting in \texttt{Moloch}. Users who
wish to reinstate it can do so by adding the following lines to their preamble.
...
Note that it may also be necessary to adjust the spacing of other elements to
accommodate this change, for instance frame titles.
...
this theme from \texttt{Moloch} because we felt that it was outside the scope
of a \texttt{beamer} theme to provide a plotting theme. The theme is still
...
\texttt{Moloch}, unlike \texttt{Metropolis}, supports subtitles. The reasoning
for this omission was that the author felt they were unnecessary clutter. We
generally agree that subtitles should be used sparingly (if at all), but we
also do not want to force this preference upon our users.

\subsection{Frame Numbering}

Metropolis sported its own frame numbering system. There was nothing wrong with
this system except it necessitated a separate package
(\texttt{appendixnumberbeamer}~\cite{lelong:2018}) to get frame or page numbers
to restart (and not count towards the total number) for appendix slides.
\texttt{beamer} has, however, improved its own system in recent years and there
is no longer any need for a custom solution (or separate package) to support
this functionality. As a result, \texttt{Moloch} just relies on beamer
templates for the frame numbering. The design is \emph{slightly} different,
with smaller font size and somewhat different margins, but the overall look is
similar.

\subsection{Block Environments}

\texttt{Metropolis} supports an alternative, filled, block type, which uses a
such that the main body text (for the frame) aligns with the left edge of the
box and not the text inside the box. Several users reported issues with this
custom handling in some context. and because we prefer the approach in
\texttt{beamer} for typographical reasons

In \texttt{Moloch} See Figure~\ref{fig:block-comparison} for a comparison.
...
does not work with every font. It does not, for instance, work with the popular
\titleref{Latin Modern}~\cite{latinmodern}, which instead generates a filled
square.
...
to switch to the \texttt{Metropolis} symbol can simply invoke the following
one-liner.
...
\begin{figure}[htb]
  ...
  \caption{%
    The design of itemize lists in Moloch (left) and Metropolis
    (right).}\label{fig:itemize-comparison}
\end{figure}

\subsection{Other Miscellaneous Changes}

\texttt{Moloch} includes a slew of other changes. For lack of space, we will
not go into all the details here. But briefly, we have
\begin{itemize}
...
\LaTeX\ packages typically come with \PDF\ documentation and
\texttt{Moloch} is no exception, featuring both a comprehensive user
guide and implementation documentation. Yet, although it is standard
to supply documentation as a \PDF\ manual, this format has some downsides.
Notably, it is not responsive and works poorly on small screens. It also
lacks good search functionality, is difficult to cross-reference outside
of the document itself, and, perhaps most importantly, is less
accessible than web documentation.

\texttt{Moloch} therefore, unlike most packages, also comes with web
documentation available at \url{https://moloch.ink}. The site features
\HTML-based versions of both the user guide and implementation documentation,
in similar fashion as the unofficial documentation for
\texttt{PGF}/\texttt{TikZ} at \url{https://tikz.dev}. Yet, whereas that site is
generated via \texttt{lwarp}~\cite{dunn:2025}, \texttt{Moloch} takes a somewhat
different approach. Since we think this approach might be of interest to other
package authors, we will briefly summarize it here.
...
and a \HTML\ website via \texttt{Quarto}. In order to provide proper styling
...
collaborate to turn this into a more reusable feature\Dash{}perhaps as a
separate package\Dash{}in order to help other authors provide web documentation
for their packages.

\section{Conclusion}

\texttt{Moloch} is developed openly on GitHub at
\url{https://github.com/jolars/moloch} and its documentation is available at
