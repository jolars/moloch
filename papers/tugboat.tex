...
\texttt{beamer} and the \LaTeX\ kernel, these these modifications became
brittle. Over time a number of issues have cropped up. Most of the issues are
minor and corrigible through workarounds, but my preamble has grown
increasingly cluttered with such patches. Moreover, a number of users have
requested features, while others have offered contributions to help improve the
theme. \texttt{Metropolis} being unmaintained, however, these contributions
have not been incorporated.

This has led me to fork \texttt{Metropolis} into a new theme, which I call
\texttt{Moloch}\footnote{Likely familiar to you if you know your Metropolis.}.
...
frames, which are intended as a mean for highlighting important frames in
...
Is it natural for \LaTeX\ packages to come with \acro{PDF} documentation, and
\texttt{Moloch} is no exception. The package features a full user manual along
with implementation documentation generated via
\texttt{docstrip}~\cite{mittelbach:2025}.

Unlike most other packages, however, \texttt{Moloch} also comes with a
\acro{HTML} documentation site, available at \url{https://moloch.ink}. The site
features web-friendly versions of both the manual and implementation
documentation in the same vein as the unofficial \acro{HTML} documentation for
\texttt{PGF}/\texttt{TikZ} at \url{https://tikz.dev}.

Whereas that site is generated via \texttt{lwarp}~\cite{dunn:2025},
\texttt{Moloch} takes a somewhat different approach. We think this approach may
be of interest to other package authors, so we will briefly outline it here.

\subsection{Pipeline}

The manual for the package is in fact written in Markdown, specifically the
flavor supported by \texttt{pandoc}~\cite{macfarlane:2025}. This allows us to
provide a single source of truth for both the \acro{PDF} and \acro{HTML}
documentation.
