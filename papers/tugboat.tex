...
\LaTeX\ \texttt{beamer}~\cite{wright:2025} presentations in addition to
...
frames~(Figure~\ref{fig:section-page-default}), but this can be modified via
\mintinline{latex}{\molochset{sectionpage=|\meta{option}|}}, with \tbcode{none}
and \tbcode{simple} for no section page and a simple section page without a
progress bar, respectively.

\begin{figure}[htb]
  \centering
  \frame{\includegraphics[page=7,width=\columnwidth]{moloch-demo}}
  \caption{%
    A section page with a progress bar in the header.
    Here we use the default progress bar line width.
  }
