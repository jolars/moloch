...
package (\texttt{appendixnumberbeamer}~\cite{lelong:2018}) to have frame
numbering restart (and not count towards the total number) for appendix slides.
\texttt{beamer} now supports this natively, so \texttt{Moloch} simply uses this
built-in functionality. The design is \emph{slightly} different, with smaller
font size and adjusted margins.
