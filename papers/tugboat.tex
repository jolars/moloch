...
\begin{abstract}
  \titleref{Moloch} is a fork of the popular \LaTeX\ \titleref{beamer} theme
  \titleref{Metropolis}, which is no longer actively maintained. Moloch aims to
  fix outstanding issues with \titleref{Metropolis} while keeping the original
  design mostly intact. This article introduces \titleref{Moloch} and summarizes
  the changes made compared to \titleref{Metropolis}.
\end{abstract}

\section{Introduction}

I have long been an admirer and user of the excellent \titleref{Metropolis}
theme~\cite{vogelgesang:2017}, which provides a clean, professional look for
\LaTeX\ \titleref{beamer}~\cite{wright:2025} presentations in addition to
...
the behavior of the theme closer to standard \titleref{beamer} behavior.
Finally, I have also made a number of internal code improvements to make the
theme more robust and maintainable going forward. Users of
\titleref{Metropolis} should find it easy to switch to \titleref{Moloch},
whilst at the same time benefit from the bug fixes, new features, and improved
stability.
...
Moloch is a standard \titleref{beamer} theme and is enabled as any other theme.
Here is a minimal example presentation using \titleref{Moloch}:
...
A popular feature of \titleref{Metropolis} (that \titleref{Moloch} retains) is
...
places a thick progress bar in the footer of each frame.
...
The results are shown in Figure~\ref{fig:progress-bars}.
