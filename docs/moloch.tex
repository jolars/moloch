...
\begin{tcolorbox}[enhanced jigsaw, bottomrule=.15mm, colback=white, colframe=quarto-callout-note-color-frame, opacityback=0, left=2mm, leftrule=.75mm, rightrule=.15mm, toprule=.15mm, breakable, arc=.35mm]

\textbf{Options:} {[}list of possible

values{]}\{style=``color: \#2a7f62''\} \textbar{} \textbf{Default:}
default
...
\textbf{Options:} {regular, smallcaps, allsmallcaps, allcaps} \textbar{}
\textbf{Default:} regular

Changes the format of ``standout'' frames (see \texttt{titleformat},
above).
...
\textbf{Options:} {none, simple, progressbar} \textbar{}
\textbf{Default:} progressbar

Adds a slide at the start of each section (\texttt{simple}) with an
...
\textbf{Options:} {none, hide, show} \textbar{} \textbf{Default:} none
...
\textbf{Options:} {none, counter, fraction} \textbar{} \textbf{Default:}
(none specified)
...
\textbf{Options:} {none, head, frametitle, foot} \textbar{}
\textbf{Default:} none

Optionally adds a progress bar to the top of each frame (\texttt{head}),
...
\textbf{Options:} {transparent, fill} \textbar{} \textbf{Default:}
transparent
...
\textbf{Options:} {dark, light} \textbar{} \textbf{Default:} light
...
{moloch} color theme is never loaded. This will prevent conflicts
between the {moloch} color theme and your preferred theme.

For example, overriding the color theme as follows may not work as
expected because
`\texttt{loads\ the\ {[}moloch{]}\{.sans-serif\}\ color\ theme,\ which\ defines\ a\ relationship\ between\ the\ frametitle\ background\ and\ the\ primary\ palette\ of\ the\ theme.\ Since}seahorse`
